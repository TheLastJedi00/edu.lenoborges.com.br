...
describe('AuthDialog', () => {
  ...
  it('preserva o e-mail digitado ao alternar entre abas', () => {
    component['loginForm'].controls.email.setValue('aluno@liga.dev');
    component.switchTab('signup');

    expect(component.tab()).toBe('signup');
    expect(component['signupForm'].controls.email.value).toBe('aluno@liga.dev');
    expect(component['signupForm'].controls.emailConfirmation.value).toBe('aluno@liga.dev');

    component['signupForm'].controls.email.setValue('outro@liga.dev');
    component.switchTab('login');

    expect(component.tab()).toBe('login');
    expect(component['loginForm'].controls.email.value).toBe('outro@liga.dev');
  });
  ...
  it('emite signup com e-mail e confirmação ao enviar formulário de cadastro', () => {
    ...
    component['signupForm'].setValue({
      email: 'novomembro@liga.dev',
      emailConfirmation: 'novomembro@liga.dev'
    });
  ...
  it('esqueci minha senha emite signup com o e-mail preenchido', () => {
    ...
    component['loginForm'].controls.email.setValue('recuperar@liga.dev');
    component['onForgotPassword']();

    expect(emittedForgot!).toEqual({
      email: 'recuperar@liga.dev',
      emailConfirmation: 'recuperar@liga.dev'
    });
  });
  ...
  it('limpa o que foi digitado ao fechar, inclusive a senha', () => {
    ...
    component['loginForm'].setValue({ email: 'alguem@liga.dev', password: 'senha-secreta' });
  ...
  it('exibe o e-mail de destino quando o estado é sent', () => {
    ...
    fixture.componentRef.setInput('sentEmail', 'destino@liga.dev');
    fixture.detectChanges();

    const compiled = fixture.nativeElement as HTMLElement;
    expect(compiled.textContent).toContain('destino@liga.dev');
