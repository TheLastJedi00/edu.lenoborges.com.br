...
- [] Task 01 (TDD): Escrever spec para o novo computed `teachingStack` de `ProfileService` antes da implementação
- [] Task 02: Estender `profile.model.ts` com `TeachingStackItem` (id, label) e remover `StackGroup`/`StackTone` (código morto) — CTAs não entram no modelo (ver alteração de escopo acima)
- [] Task 03: Atualizar `PROFILE` em `profile.service.ts` — identity (role/tagline/summary focados em professor particular), lista das 8 stacks ensinadas, remove o campo `stack` antigo, prova social dev condensada
...
- [] Task 02: Criar componente dumb `TeachingStackGrid` (grid de ícone + label das 8 stacks ensinadas) e remover `StackGroupCard` (código morto, substituído)
