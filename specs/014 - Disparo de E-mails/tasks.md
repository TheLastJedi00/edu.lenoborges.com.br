...
- [ ] Task 01: O modelo. Arquivo: `src/app/models/email.model.ts`. Objetivo: `EmailFilters`
...
- [ ] Task 02 (TDD + implementação): O serviço. Arquivos: `src/app/services/email.service.ts`, `.spec.ts`.
...
- [ ] Task 03 (TDD + implementação): O descadastro. Arquivos: `src/app/services/email.service.ts`,
...
- [ ] Task 04 (TDD + implementação): O interruptor do perfil. Arquivos:
...
- [ ] Task 05: `emailOptOut` no `AdminUser`. Arquivo: `src/app/models/admin.model.ts`. Objetivo: o campo
  novo, para a lista de usuários poder mostrar quem não recebe.

# Fase 02: Escrever e filtrar [ ]
...
- [ ] Task 01: Rota e porta. Arquivos: `src/app/app.routes.ts`, `src/app/pages/admin/admin.page.ts`.
  ...
  por uma porta só.
- [ ] Task 02: O bloco Escrever. Arquivos: `src/app/pages/admin/emails/emails.page.ts`, `.html`, `.scss`.
...
- [ ] Task 03: O bloco Para quem. Objetivo: os quatro tiers como caixas (rótulos do `billing.model`, não
...
- [ ] Task 04 (TDD + implementação): A contagem viva. Objetivo: mudança de filtro dispara
...
- [ ] Task 05 (TDD + implementação): Falha de audiência bloqueia. Objetivo: erro em `audiencia` põe um
...
- [ ] Task 06: A prévia. Objetivo: o corpo renderizado com o mesmo espaçamento e a mesma hierarquia do
...
- [ ] Task 03: O botão diz o número. Objetivo: **"Enviar para 42 pessoas"**, nunca "Enviar" (decisão 4).
...
- [ ] Task 04 (TDD + implementação): A confirmação. Objetivo: o `confirm-dialog` já existente, com o
...
- [ ] Task 05 (TDD + implementação): O envio. Objetivo: formulário desabilitado, botão em **"Enviando…"**,
...
- [ ] Task 06 (TDD + implementação): O erro que **não** diz "não enviou". Objetivo: falha de rede ou de
...
- [ ] Task 07: Mobile. Objetivo: campos em coluna, filtros empilhados, alvos de 44px, e o botão de envio
...
- [ ] Task 01: A lista. Objetivo: as 20 campanhas mais recentes com assunto, data, "para N pessoas" e o
...
- [ ] Task 02 (TDD + implementação): Retomar. Objetivo: campanha `interrompida` ganha o botão
  **"Retomar"**, que chama `retomar(id)` e atualiza a linha. Enquanto retoma, a linha fica em estado de
  ...
  por 409.
- [ ] Task 03: Estado vazio. Objetivo: **"Nenhum e-mail enviado ainda"**, no mesmo tom do "Nada novo por
  aqui" da spec 012.
- [ ] Task 04: Uma requisição só ao abrir. Objetivo: `listar()` na inicialização, e de novo depois de um
  envio bem-sucedido. **Nada de polling** — a mesma régua das specs 010, 011 e 012.

# Fase 05: O interruptor em Meu Perfil [ ]
Branch: `feat/014-perfil-emails`

> Depende da spec 013 estar no código. Ver a nota no topo.

- [ ] Task 01: A seção. Arquivos: `src/app/pages/perfil/perfil.page.html`, `.scss`. Objetivo: seção
...
- [ ] Task 02 (TDD + implementação): Salvar sozinho. Objetivo: o interruptor salva no gesto, otimista,
...
- [ ] Task 03: O estado inicial. Objetivo: o interruptor nasce do `emailOptOut` que vem do `GET /me`.
...
- [ ] Task 04: Quem foi descadastrado por bounce religa aqui, e a tela não explica por quê (decisão 12).
  Objetivo: nenhum texto especial, nenhum aviso. Está escrito como task para que a ausência seja
  deliberada, e não esquecimento.

# Fase 06: A página pública de descadastro [ ]
Branch: `feat/014-descadastro`

- [ ] Task 01: Rota pública. Arquivo: `src/app/app.routes.ts`. Objetivo: `/descadastro`, **sem guard
...
- [ ] Task 02 (TDD + implementação): A página. Arquivos: `src/app/pages/descadastro/descadastro.page.ts`,
  ...
  rodapé já confirmou.
- [ ] Task 03: Sem token. Objetivo: sem `?token=`, a tela explica em uma frase que o link está incompleto
  e manda entrar em Meu Perfil. Sem toast e sem erro vermelho: quem chega aqui não fez nada errado.
- [ ] Task 04 (TDD): Não depende de sessão. Objetivo: teste-trava de que a página renderiza e completa a
...
- [ ] Task 05: Título e `noindex`. Objetivo: título próprio e a página fora dos buscadores. É uma URL com
...
- [ ] Task 01: O aviso na trilha. Arquivo:
...
- [ ] Task 02: Quem não recebe, na lista de usuários. Arquivos:
