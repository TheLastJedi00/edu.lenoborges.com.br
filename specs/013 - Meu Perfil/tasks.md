...
- [x] Task 01: O input de alinhamento. Arquivos:
...
- [x] Task 02: A regra. Arquivo: `notification-panel.ts`. Objetivo:
  ...
  nada nela muda.
- [x] Task 03: O aside declara o lado. Arquivo:
...
- [x] Task 04 (TDD): Spec do alinhamento. Objetivo: teste-trava de que a instância do aside recebe
...
- [x] Task 01: Os campos no modelo. Arquivo: `src/app/models/auth.model.ts`. Objetivo: `linkedin` e
...
- [x] Task 02 (TDD + implementação): Normalizar rede social. Arquivos:
...
- [x] Task 03 (TDD + implementação): `updateProfile` com as redes. Arquivos:
...
- [x] Task 04 (TDD + implementação): `changeEmail`. Arquivos: `src/app/core/auth/auth.service.ts`,
...
- [x] Task 05 (TDD + implementação): `changePassword`. Arquivos: `src/app/core/auth/auth.service.ts`,
  ...
  errou a digitação.
- [x] Task 06 (TDD + implementação): `deleteAccount`. Arquivos: `src/app/core/auth/auth.service.ts`,
...
# Fase 02: A rota e a casca da tela [x]
Branch: `feat/013-rota-perfil`

- [x] Task 01: A rota. Arquivo: `src/app/app.routes.ts`. Objetivo: `/dashboard/perfil` com
  `authGuard` + `profileCompleteGuard`, título `Meu Perfil · Liga Dev`, `loadComponent`. Nas mesmas
  condições das irmãs — nenhum guard novo.
- [x] Task 02: Destravar o cartão do painel. Arquivo: `src/app/pages/dashboard/dashboard.page.html`.
...
- [x] Task 03: Destravar o item do aside. Arquivo:
...
- [x] Task 04: Casca da página. Arquivos:
...
- [x] Task 05: Estado de carregando. Objetivo: enquanto o `GET /me` não voltar, os campos não aparecem
...
# Fase 03: Seus dados [x]
Branch: `feat/013-seus-dados`

- [x] Task 01: O formulário. Objetivo: nome, telefone e bio, com **as mesmas validações do
...
- [x] Task 02: Salvar. Objetivo: botão desabilitado enquanto `form.pristine` ou `form.invalid` (decisão
...
- [x] Task 03: A linha sobre o nome. Objetivo: abaixo do campo de nome, discreta: as perguntas já
  publicadas no Mural continuam com o nome antigo (decisão 3). **Uma linha, sem ícone de aviso** — é
  informação, não alerta.
- [x] Task 04 (TDD): Sair com alteração não salva. Arquivos: `perfil.page.ts`, `.spec.ts`. Objetivo:
...
- [x] Task 05 (TDD): Spec da seção. Objetivo: pré-preenchimento vindo do `GET /me`, botão travado sem
  alteração, salvar atualizando o store, e erro de rede aparecendo **abaixo do formulário** sem navegar
  (decisão 11).

# Fase 04: Suas redes [x]
Branch: `feat/013-redes`

- [x] Task 01: Os dois campos. Objetivo: LinkedIn e Instagram, **opcionais**, num formulário com botão
  próprio (decisão 1). Ícones vindos de `icon-linkedin.ts` e `icon-instagram.ts`, que já existem — nenhum
  ícone novo.
- [x] Task 02: Normalizar ao sair do campo. Objetivo: `@fulano` vira a URL completa no próprio input, no
  `blur` (decisão 4). A pessoa vê no que o texto dela virou antes de salvar, em vez de descobrir depois.
- [x] Task 03: Apagar remove. Objetivo: campo esvaziado manda string vazia, e a API grava `null`. Depois
...
- [x] Task 04 (TDD): Spec da seção. Objetivo: os três formatos de entrada aceitos, domínio errado
...
# Fase 05: Acesso — e-mail e senha [x]
Branch: `feat/013-acesso`

- [x] Task 01: Os dois blocos fechados. Objetivo: cada um mostra só o estado — o e-mail atual, e "Senha ·
  alterada por você" sem nenhum dado — e abre ao toque (decisão 5). Fechar cancela e limpa os campos,
  **inclusive os de senha**.
- [x] Task 02: A explicação do campo de senha atual. Objetivo: "Confirmamos que é você antes de mudar o
...
- [x] Task 03: Trocar de e-mail. Objetivo: novo e-mail + senha atual, `POST /me/email`. No `202`, o bloco
...
- [x] Task 04 (TDD): O e-mail na tela não muda. Objetivo: teste-trava de que o `202` **não** altera o
...
- [x] Task 05: Trocar de senha. Objetivo: senha atual + nova + confirmação da nova. A confirmação é do
  front e não vai para a API — é proteção contra digitar errado a senha que ninguém vai lembrar depois.
- [x] Task 06: O aviso antes do botão. Objetivo: *"Ao trocar a senha, você sai de todos os aparelhos e
...
- [x] Task 07: O destino depois do `204`. Objetivo: `clearSession()`, navegar para `/?entrar=1` — o
...
- [x] Task 08 (TDD): Spec da seção. Objetivo: `401` mostrando "Senha incorreta." **dentro do bloco**, sem
...
- [x] Task 01: A seção. Objetivo: no fim da tela, separada por respiro grande, com borda de atenção e
  **sem vermelho gritante** (decisão 8). O vermelho é do botão final, dentro do diálogo.
- [x] Task 02: A lista do que some e do que fica. Objetivo: as duas colunas da decisão 8, literais, mais
...
- [x] Task 03: O diálogo. Arquivo: `src/app/pages/perfil/perfil.page.html` + `dialog-box` existente.
  Objetivo: resumo em uma frase, campo de senha, **Cancelar** e **Excluir minha conta**. Botão final
  desabilitado com o campo vazio.
- [x] Task 04 (TDD): O foco inicial é o Cancelar. Objetivo: teste-trava. É a única tela do produto onde o
...
- [x] Task 05: Excluir. Objetivo: `DELETE /me`, e no `204` `clearSession()` + `router.navigate(['/'])`.
  **Sem toast de sucesso na landing** (decisão 10) — comemorar a saída de alguém é fora de hora.
- [x] Task 06: Falhar mantém o diálogo aberto. Objetivo: `401` vira "Senha incorreta." dentro do diálogo;
...
- [x] Task 07 (TDD): Spec da exclusão. Objetivo: botão travado sem senha, `401` sem deslogar e sem
...
- [x] Task 01: A tela no celular. Objetivo: seções empilhadas, campos com largura cheia, alvos de toque
  de **44px de verdade** — a mesma régua da decisão 11 da spec 010 e da decisão 13 da spec 012.
- [x] Task 02: `autocomplete` correto em cada campo. Objetivo: `name`, `tel`, `email`,
...
- [x] Task 03: O diálogo de exclusão no celular. Objetivo: largura de folha, altura que não corta os
...
- [x] Task 01: `README.md`. Objetivo: a tela em duas linhas, e uma frase sobre exclusão de conta — o que
  some, o que fica anônimo, e que é imediato.
- [x] Task 02: Revogar os "Em breve" nas specs antigas. Objetivo: onde a spec 005 registrar que Meu
