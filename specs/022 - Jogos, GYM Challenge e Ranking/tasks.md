...
# Fase 01: Modelos, serviços e o nickname no AuthStore []

- [] Task 01: `src/app/models/games.model.ts` — todos os tipos da spec: `ChallengeStatus`,
...
- [] Task 02: `src/app/services/games.service.spec.ts` — **testes antes**, com `HttpTestingController`:
...
- [] Task 03: `src/app/services/games.service.ts` — `providedIn: 'root'`, `inject(HttpClient)`,
  no molde do `track.service.ts`.
- [] Task 04: `src/app/services/ranking.service.spec.ts` e `ranking.service.ts` — `page({ limit, after })`
  com `HttpParams`, devolvendo `{ entries, myPosition, myEntry }`.
- [] Task 05: `src/app/models/auth.model.ts` — `nickname: string | null` no `MemberProfile`. Vem do
  `GET /me` pelo caminho que o `xp` da spec 019 já usa.
- [] Task 06: `src/app/services/profile.service.ts` + `.spec.ts` — `setNickname(nickname)` chamando
  `PUT /me/nickname`, e o `409` propagado como erro tipado para a tela distinguir "nome em uso" de
  "você já tem um".
- [] Task 07: `src/app/core/auth/auth.store.ts` + `.spec.ts` — expor o signal `nickname` e o
  `setNickname(value)` local, no molde do `setXp` da spec 019.

---

# Fase 02: O aside, o hub e as rotas []

- [] Task 01: `src/app/components/dashboard-aside/dashboard-aside.ts` — o item Jogos perde o `disabled`,
...
- [] Task 02: `dashboard-aside.spec.ts` — atualizar. O teste que hoje afirma o item desabilitado passa a
  afirmar o link ativo, na posição certa.
- [] Task 03: `src/app/app.routes.ts` — o nó `jogos` dentro de `dashboard`, com as quatro rotas filhas
...
- [] Task 04: `src/app/pages/jogos/jogos.page.ts` + `.html` + `.scss` — o hub com os três cards da
...
- [] Task 05: `jogos.page.spec.ts` — os três cards, o Duels sem `routerLink` e com `aria-disabled`,
  e os dois links ativos apontando certo.

---

# Fase 03: O modal de Gamertag e o guard []

- [] Task 01: `src/app/components/nickname-dialog/nickname-dialog.ts` + `.scss` — modal bloqueante no molde
...
- [] Task 02: `nickname-dialog.spec.ts` — botão travado enquanto inválido, mensagem de erro no `409`,
  `animate-enter` / `animate-leave` presentes, e o foco preso no modal.
- [] Task 03: `src/app/core/games/nickname.guard.ts` + `.spec.ts` — lê o `AuthStore`; com `nickname` nulo,
...
- [] Task 04: `src/app/pages/perfil/perfil.page.*` — o campo Gamertag na aba Meu Perfil, `disabled`
  quando já preenchido, com o texto explicando a imutabilidade. Atualizar `perfil.page.spec.ts`.

---

# Fase 04: O card do GYM Challenge e a lista de desafios []

- [] Task 01: `src/app/components/gym-challenge-card/gym-challenge-card.ts` + `.scss` — o componente burro
  ...
  não faz requisição.
- [] Task 02: `gym-challenge-card.scss` — a barra de progresso de XP com `width` proporcional e
...
- [] Task 03: `src/app/components/round-dots/` — as 3 bolinhas de rodada (aprovada / pendente / atual),
  componentizadas porque aparecem no card, na lista e na tela do desafio.
- [] Task 04: `gym-challenge-card.spec.ts` — os quatro estados, o card de "em breve" não clicável, o texto
  de XP faltante calculado a partir do que o servidor mandou, e o `prefers-reduced-motion`.
- [] Task 05: `src/app/pages/jogos/desafios/desafios.page.ts` + `.html` + `.scss` — smart page: chama
  `GET /games/challenges` e renderiza as 8 insígnias com o card. Ícone da insígnia reaproveitado da trilha.
- [] Task 06: `desafios.page.spec.ts` — carregamento, estado de erro, e o clique navegando para
  `/dashboard/jogos/desafio/:badgeId`.

---

# Fase 05: A tela do desafio []

- [] Task 01: `src/app/pages/jogos/desafio/desafio.page.ts` — o esqueleto com o `signal<Fase>` das cinco
  ...
  telas sobrepostas.
- [] Task 02: `src/app/components/challenge-warning/` — a tela intersticial da decisão 4, com o título
...
- [] Task 03: `challenge-warning.spec.ts` — botão travado sem o checkbox, e o checkbox voltando a
  desmarcado a cada nova rodada.
- [] Task 04: `src/app/components/challenge-timer/` — o timer visual: `setInterval` de 100ms alimentando um
...
- [] Task 05: `src/app/pages/jogos/desafio/answer-clock.ts` + `.spec.ts` — a medição, separada do timer
  ...
  **Nunca derivada do contador visual**, que acumula erro de `setInterval` e é estrangulado em aba de fundo
  (adendo A.5).
- [] Task 06: `src/app/components/question-view/` — o enunciado e as quatro alternativas: coluna no mobile,
  2x2 no desktop, alvo de toque de 48px, texto que quebra em linhas (decisão 14). Emite o índice escolhido.
- [] Task 07: `question-view.scss` — o feedback da decisão 15: escolhida certa pulsa verde, escolhida errada
  pulsa vermelho com a correta destacada, o `+XP` sobe em fade, tudo em 200ms. Botão inferior ancorado com
  `env(safe-area-inset-bottom)`.
- [] Task 08: `desafio.page.ts` — a fase `jogando`: `POST .../start`, as 10 questões em memória, uma por
...
- [] Task 09: `src/app/components/round-result/` — a fase `resultado`: "Você acertou 8 de 10!", o XP da
...
- [] Task 10: O selo "Modo Treino: Sem XP" perto do timer quando o `start` ou o `answer` devolve
  `replay: true`, e o acerto sem a animação de subida de XP (decisão 17).
- [] Task 11: `desafio.page.spec.ts` — o fluxo completo com o service mockado: aviso obrigatório, as 10
...
- [] Task 01: `src/app/components/podium/` — os três cards do pódio da decisão 6: dourado, prateado e
  bronze, lado a lado no desktop e empilhados no mobile. **Cores sólidas, nada pisca, sem loop.**
- [] Task 02: `src/app/components/position-delta/` — o selo de evolução diária (↑ verde / ↓ vermelho),
...
- [] Task 03: `src/app/pages/jogos/ranking/ranking.page.ts` + `.html` + `.scss` — o pódio, a lista do 4º em
...
- [] Task 04: Paginação por "Carregar mais" com o cursor `after`, acumulando no signal — nunca paginação
  por número de página.
- [] Task 05: `ranking.page.scss` — a entrada em cascata com `appReveal`, **com teto de 6 linhas**
  (decisão 15), e `prefers-reduced-motion` desligando.
- [] Task 06: `ranking.page.spec.ts` — a ordem, o destaque da própria linha, o "Carregar mais" pedindo a
  página seguinte, e a posição do membro no topo mesmo estando na página 3.

---

# Fase 07: A emenda na trilha []

- [] Task 01: `src/app/pages/trilha/insignia/insignia.page.ts` — passa a chamar
  `GET /games/challenges/:badgeId` junto do `GET /badges/:badgeId/videos`, **num `Promise.all`**
  (regra 8 do `clauderc`).
- [] Task 02: `insignia.page.html` — o `gym-challenge-card` com `emphasis` ligado, na aba Aulas,
...
- [] Task 03: `insignia.page.spec.ts` — o card presente e em último lugar, a aba Perguntas sem ele, e a
...
- [] Task 01: `src/app/services/admin.service.ts` + `.spec.ts` — **estendido, não duplicado**: as oito
  rotas de `/admin/badges/:badgeId/questions` e `/challenge-config` entram no serviço de admin que já
  existe (adendo A.3).
- [] Task 02: `src/app/pages/admin/questoes/questoes-admin.page.ts` — as treze insígnias como cards
  selecionáveis, no molde exato de `pages/admin/trilha/trilha-admin.page.ts` (decisão 9).
- [] Task 03: `src/app/app.routes.ts` — `admin/questoes` e `admin/questoes/:badgeId` com `adminGuard`,
  no molde das rotas de `admin/trilha`.
- [] Task 04: `src/app/pages/admin/admin.page.*` — o cartão de entrada para a nova seção.
- [] Task 05: `src/app/pages/admin/questoes/insignia-questoes.page.ts` + `.html` + `.scss` — a página da
...
- [] Task 06: `src/app/components/question-editor/` — o formulário de questão (enunciado, 4 alternativas,
...
- [] Task 07: A lista de questões: enunciado truncado em 2 linhas, dificuldade, as 4 alternativas com a
  correta em verde, e as ações Editar e Excluir. A exclusão passa pelo `ConfirmDialog` que já existe.
- [] Task 08: A seção de configuração no topo (decisão 11): input de XP mínimo, o status
...
- [] Task 09: `insignia-questoes.page.spec.ts` — os contadores, o filtro por aba, o CRUD completo com o
  service mockado, e o estado "pronto para publicação".

---

# Fase 09: Geração com IA []

- [] Task 01: `src/app/components/ai-generate-dialog/ai-generate-dialog.ts` + `.scss` — o modal de dois
  passos da decisão 10. Passo 1: textarea do prompt, select de dificuldade, select de quantidade
  (10 / 20 / 30) e o botão Gerar.
- [] Task 02: O estado de loading com **esqueleto de questões**, não spinner — a espera da API é longa e
  o esqueleto diz o que está vindo.
- [] Task 03: Passo 2 — a lista revisável, cada questão com o `question-editor` da Fase 08, o checkbox de
  seleção e o botão de excluir individual. Sem "regenerar" por questão (ponto em aberto 4, fechado: não).
- [] Task 04: A barra de ação fixa no rodapé: "Selecionadas: 28/30" e o botão "Salvar Selecionadas"
  chamando `POST .../questions/bulk`.
- [] Task 05: O aviso de descarte ao fechar com rascunho não salvo, usando o `ConfirmDialog` existente —
  **não** o `unsavedChangesGuard`, que é de rota e aqui não há troca de rota (adendo A.9).
- [] Task 06: `ai-generate-dialog.spec.ts` — os dois passos, a edição antes de salvar, a contagem de
  selecionadas, o corpo do `bulk` levando só as marcadas, e o aviso de descarte no fechamento.

---

# Fase 10: Acessibilidade, movimento e fechamento []

- [] Task 01: Varredura de `prefers-reduced-motion` em todos os `.scss` desta spec — card, timer, feedback,
  conquista e cascata do ranking. É a regra de todas as specs, e é a que mais escapa em `.scss` novo.
- [] Task 02: Acessibilidade do questionário: as alternativas são `<button>` com `aria-pressed`, o
...
- [] Task 03: Passada mobile no Chrome, largura de 360px: alvo de toque de 48px nas alternativas, o botão
  inferior com `env(safe-area-inset-bottom)`, o pódio empilhado e a tabela do ranking sem estouro horizontal.
- [] Task 04: Conferir que nenhuma tela desta spec grava em `localStorage` (decisão 12) e que nenhuma
  calcula XP ou conhece `correctIndex` fora do que o servidor mandou (decisão 13).
- [] Task 05: `npm run lint` e `npm test` limpos, e a passada final no Chrome no fluxo inteiro:
  aside → hub → gamertag → desafios → uma rodada completa → ranking → trilha com o card → admin de questões.
- [] Task 06: Marcar as emendas nas specs afetadas: **008** (Jogos sai do "Em breve"), **009** (trilha e
